...
% slightly modified by Artem Diuba
 
function SGLXMetaToCoords_AD(varargin)
...
switch outType
    ...
    case 1     %KS2 *.mat
        ...
        name = fname;
        % savepath = uigetdir('C:\NeuralData\Data\Linguine_data_sample\supercat', 'Select AP Metadata File');
        if ask_savepath
            savepath = uigetdir(path, 'Select AP Metadata File');
        end
        save( fullfile(savepath, newName), 'chanMap', 'chanMap0ind', 'connected', 'name', 'xcoords', 'ycoords', 'kcoords' );

        if bPlot
            savefig(gcf, fullfile(savepath, [newName(1 : end - 4), '.fig']));
            print(gcf, '-dpng', fullfile(savepath, [newName(1 : end - 4), '.png']))
        end
